...
struct JournalEntryCreatorView: View {
  ...
  var body: some View {
    ...
    .onAppear {
      NotificationCenter.default.addObserver(
        forName: UIApplication.keyboardWillShowNotification, object: nil, queue: nil
      ) { _ in
      ...
      NotificationCenter.default.addObserver(
        forName: UIApplication.keyboardWillHideNotification, object: nil, queue: nil
      ) { _ in
        DispatchQueue.main.async {
    ...
    Button {
      if currentStep == .shoot {
        if let level = moodLevel, !todayText.isEmpty, !reboundText.isEmpty, !text.isEmpty {
          manager.saveEntry(
            text: text, moodLevel: level.rawValue, moodText: todayText, reboundText: reboundText,
            reasons: reasons, images: images)
          if isRebounded {
          ...
          presentAlert(
            title: "Missing Fields", message: "Make sure that you've complete all required fields")
        }
      } else {
        ...
        ScrollView(.vertical, showsIndicators: false) {
          ForEach(MoodReason.allCases) { reason in
            HStack {
              ...
              Spacer()
              Image(systemName: reasons.contains(reason) ? "checkmark.square.fill" : "square").font(
                .system(size: 20))
            }
            .padding().background(.list)
      ...
      ZStack {
        if images.count == 0 {
          ...
          HStack(spacing: 5) {
            ...
            PhotoContainer(
              image: placeholder, index: 1, width: width / 2 - 5, height: width / 2 - 5)
          }
        } else if images.count == 2 {
          HStack(spacing: 5) {
            VStack(spacing: 5) {
              PhotoContainer(
                image: images[0], index: 0, width: width / 2 - 5, height: width / 2 - 5)
              PhotoContainer(
                image: images[1], index: 1, width: width / 2 - 5, height: width / 2 - 5)
            }
          ...
          HStack(spacing: 5) {
            ...
            if images.count == 3 {
              VStack(spacing: 5) {
                PhotoContainer(
                  image: images[2], index: 2, width: width / 2 - 5, height: width / 2 - 5)
                PhotoContainer(
                  image: placeholder, index: 3, width: width / 2 - 5, height: width / 2 - 5)
              }
            } else {
              VStack(spacing: 5) {
                PhotoContainer(
                  image: images[2], index: 2, width: width / 2 - 5, height: width / 2 - 5)
                PhotoContainer(
                  image: images[3], index: 3, width: width / 2 - 5, height: width / 2 - 5)
              }
            }
          }
        }
  ...
  private func PhotoContainer(image: UIImage, index: Int, width: CGFloat, height: CGFloat)
    -> some View
  {
    func presentPhotoPickerAlert(_ index: Int) {
      selectedPhotoIndex = index
    ...
    return Image(uiImage: image.resizeImage(newWidth: width)).resizable().aspectRatio(
      contentMode: .fill
    )
